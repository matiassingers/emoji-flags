...
describe('getter methods', function(){
  it('data', function() {
    assert.equal(emojiFlags.data.length, 250);
  ...
  it('emojis', function() {
    assert.equal(emojiFlags.emojis.length, 250);

    assert.equal(emojiFlags.emojis[77], '🇬🇧');
  });

  it('codes', function() {
    assert.equal(emojiFlags.codes.length, 250);

    assert.equal(emojiFlags.codes[201], 'SJ');
  });

  it('names', function() {
    assert.equal(emojiFlags.names.length, 250);

    assert.equal(emojiFlags.names[151], 'Mauritania');
  });

  it('unicodes', function() {
    assert.equal(emojiFlags.unicodes.length, 250);
